...
4. **Codex inbound:** provenance rides inside the `{type:"text",text}` item as a
   machine-generated header, a receiver-generated unguessable per-delivery delimiter,
   then the untrusted body (§5).
...
| `oac_device` | The sending device's Ed25519 public-key fingerprint — the same fingerprint construction `security.key_id` uses (`C5-envelope-auth.md` §4/§10(b), cited in this row's Cite column), never the display URI's `<device>` segment. | `docs/planning/decisions/C5-envelope-auth.md` §4 ("Public half is the device identity"), §10(b) (fingerprint construction, 128-bit floor). |
...
2. **Contract test asserting every emitted key matches `^[A-Za-z0-9_]+$`.** A test in the
   Claude adapter's contract suite (`oac-testing`) — backlog tasks G4/G5 (Claude adapter)
   already require this contract suite per their own acceptance criteria — constructs a
   `meta` map through the
...
1. **Machine-generated header block** — the same five fields §2 names, same names
   ...
   requirement).
2. **An unguessable-per-delivery delimiter, generated by the receiving adapter**,
   opening and closing the untrusted body section.
...
**Delimiter derivation — the threat actor is the sending peer, not a third party, so
the delimiter must not come from anything the sender controls or can compute.** An
earlier draft of this section derived the delimiter from the envelope's own
`security.nonce`. That is wrong against the actor this document's own §12 row names: the
sender generates its own nonce and signs its own envelope (`docs/planning/decisions/
C5-envelope-auth.md` §4, per-device signing keys; §7, "a fresh nonce is generated for
every signed envelope" — generated by the sender, before it signs). A malicious sender
can therefore compute the nonce-derived delimiter itself, before signing, and compose a
body containing `--- oac-body <that delimiter> ---` plus a forged header block, then sign
the envelope normally. The signature verifies; nothing about §5's frame shape catches the
forgery. Deriving the delimiter from a signed-but-sender-generated field gives no
unguessability against the sender, only against an outside observer who is not this
document's threat actor.

**Corrected mechanism: the delimiter is generated by the *receiving* adapter, from its
own CSPRNG, at render/delivery time — never derived from, or computable from, any field
of the envelope itself.** Concretely: when the Codex adapter is about to frame a
delivered envelope into the §5 shape, it draws a fresh random value (128 bits (16 bytes)
from the OS CSPRNG, encoded the same way OAC already encodes random identifiers
elsewhere in this design — 26-character lowercase Crockford Base32, matching `oac_sender`
§2's encoding) and uses that value, and only that value, as `<delimiter>` for this one
delivery. No sender, malicious or not, can know this value before it is generated,
because it is not a function of anything the sender sent — it exists only in the
receiving adapter's own process state, generated after the signed envelope has already
arrived and been verified.

**Defense in depth: the adapter also scans the body for a collision before framing.**
Before emitting the frame, the adapter checks whether the envelope's rendered `content`
already contains the literal substring `--- oac-body ` or `--- oac-envelope ` or
`--- oac-end ` followed by the freshly-generated delimiter value. Because the delimiter
is generated after the content is already known to the adapter, this check is exact, not
probabilistic-with-timing-risk: on the vanishingly unlikely event of a collision (the
128-bit value happening to already appear in the body), the adapter regenerates a new
delimiter and re-checks, rather than framing with a colliding value.

**This is what makes it impossible for body content to forge a header or close the fence
early:** a sender composing a message body has no channel to learn the delimiter this
specific delivery will use, because that value does not exist until the receiving
adapter generates it — after the sender has already sent, and after the sender-controlled
part of the pipeline (composing and signing the envelope) is finished. The unguessability
property this design needs holds against a malicious *sender*, which is the actor
`oac-security-work`'s "malicious peer prompt injection" doctrine and this document's own
§12 row 2 name, because the generating party and the sending party are no longer the
same party.
...
`turn/steer`. This is the backlog task G7 / `oac-security-work` §2 boundary, quoted:
"`turn/steer` is either unused or gated behind an explicit authorization check" — the
framing this section defines assumes the "unused" branch for ordinary inbound message
delivery; if a future decision routes any inbound content into `turn/steer`, that routing
needs backlog task G7's own separate authorization gate (`docs/planning/decisions/
C5-envelope-auth.md` §11's
...
- **Machine-set:** §2's `meta` values and §5's header-block values are both produced by
  ...
  receiver-generated, sender-unpredictable delimiter).
...
(`docs/planning/ADR-001-AMENDMENTS.md`: "C5 | Claude needs legacy MCP; Codex
...
|---|---|---|---|---|
| Provenance spoofing via message body on Claude | Attacker controls a validly-signed envelope's `content` text and crafts it to look like a `<channel>` tag or forged `meta`-shaped claim | `content`/`meta` are separate wire fields on `notifications/claude/channel`; `meta` is machine-set from daemon state, never content-derived (§2, §4) | gate G1 (Claude wake, exercises real rendering); gate G5 (provenance) | gate G1/gate G5 `NOT RUN`; designed, not proven, mitigation |
| Same, on Codex via forged header/delimiter | Attacker (the malicious sending peer itself, per `oac-security-work`'s "malicious peer prompt injection" actor) crafts a message body containing text shaped like §5's header block or delimiter, hoping to make a reader (the model, or a downstream parser) treat it as the real header | Delimiter is generated by the *receiving* adapter from its own CSPRNG at render/delivery time — never derived from any sender-controlled or sender-computable envelope field — so the sending peer cannot know the delimiter value before, during, or after composing the body; the adapter also scans the body for a collision against the generated value before framing and regenerates on collision (§5) | gate G2 (Codex live inject); gate G5 (provenance) | gate G2/gate G5 `NOT RUN`; residual risk if a future implementation ever generates or exposes the delimiter to sender-side code (e.g. an adapter that lets a sender pre-view its own framed delivery) — this design assumes the delimiter is generated strictly after, and by a different party than, envelope composition, and that assumption is not yet test-enforced |
| Silently dropped `meta` key yielding unlabelled provenance | A bug emits a non-identifier-safe `meta` key | Const key table (§3) plus the refusal fixture: incomplete provenance is detected pre-send and the message is refused, not delivered unlabelled (§3) | Contract test (§3 step 2); refusal fixture (§3 step 3) | Both tests do not exist yet (not yet built, no implementation); moves to the risk list until backlog tasks F8/G4 build them |
| Unauthorized tool approval via permission relay | `claude/channel/permission` is enabled for a deployment | Off by default in v0.1 (§7); enabling it requires its own separate decision, never a side effect of allowlisting | backlog task G4 acceptance ("Permission relay is off by default"); backlog task H2 acceptance ("Permission relay is confirmed off... documented") | backlog tasks G4/H2 not yet built; a deployment that opts in accepts the documented consequence (§7) — this is an accepted, named risk for that deployment, not eliminated by this table |
| Reply misattribution via forged `in_reply_to` | Attacker's message content instructs the model to claim a specific, incorrect `in_reply_to` value | (a) alone is never trusted; validated against (b)'s independently-tracked thread-id/turn-id binding; a mismatch downgrades to inferred/uncorrelated, never silently accepted (§10) | backlog task G8 (Codex adapter reply-correlation acceptance, "tested against a reply that omits the correlation hint") | backlog task G8 not yet built; whether Codex reliably reproduces a header-supplied id in a tool call at all is itself UNVERIFIED (§13) — a spike, not this document, resolves it |
...
- **A fixed static delimiter for the Codex body.** Rejected: a static, predictable
  ...
  pre-planted inside a message body, letting an attacker craft content that closes the
  fence early or forges a second header. **This is the same defect an earlier draft of
  §5 had with a sender-nonce-derived delimiter** — the sender generates its own nonce
  and signs its own envelope (`docs/planning/decisions/C5-envelope-auth.md` §4, §7), so a
  value derived from the sender's own nonce is just as computable-in-advance by that
  sender as a hardcoded string is; "signed" does not mean "unknown to the party doing the
  signing." What actually distinguishes the chosen design from this rejected one is not
  that the delimiter is per-message rather than fixed — it is that the delimiter is
  generated by the *receiving* adapter, after the envelope arrives, from a value the
  sender never sees or computes (§5's corrected derivation). A per-message value the
  sender itself can still compute would be rejected for the identical reason a fixed
  value is.
...
- [x] Codex: text-input framing with a machine-generated header and a delimited
      untrusted body — §5 (header block, receiver-generated delimiter, framed example,
