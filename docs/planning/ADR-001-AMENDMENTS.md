...
| C4 | DESIGN envelope `security.signature` "implementation-defined" vs acceptance criterion 6 (enforced provenance) | RESOLVED-IN-DECISION | `docs/planning/decisions/C5-envelope-auth.md` §6 (issue #18) — signature is normative (Ed25519 via `ed25519-dalek`, `verify_strict`, over a domain-separated JCS canonicalization); replacement text for `DESIGN.md`'s `security` block given; normative MUST lands in a future `spec/security.md` (Epic E); no `ADR-001.md` text needs correction (it carries no `implementation-defined` text at all), so no new numbered amendment is issued | PLANNING-PROMPT.md §3.4 / `REVERIFICATION-B2.md` §3.4 — "Zenoh provides no application-layer message signing," so the envelope signature is the only authenticity proof |
...
| C6 | No Claude acknowledgement vs DESIGN `accepted` delivery state | RESOLVED-IN-DECISION | `docs/planning/decisions/C5-envelope-auth.md` §9 (issue #18) — three honest receipt states (`accepted-by-adapter`, `handed-to-harness`, `unknown`) mapped onto DESIGN's delivery-state list; a resolved Claude notification send is `handed-to-harness`, never asserted as "seen by the model" | PLANNING-PROMPT.md §3.1, "Claude Code sends no acknowledgement; a resolved notification send means 'written to transport', not 'seen by the model'"; re-verified `REVERIFICATION-B2.md` §3.1 table row "No acknowledgement of delivery" — HOLDS |
| C7 | ACP is client-owned-session, not a channel | RESOLVED-BY-EVIDENCE | `docs/planning/v0.1/01-capability-matrix.md` §4 (Epic A task A2) | `REVERIFICATION-B2.md` §3.5 row "C7 framing: ACP is client-owned-session, not a channel" — HOLDS. ACP is not a v0.1 dependency; ACP schema v2 "alpha" remains UNVERIFIED (`STATUS.md` open list) |
...
| C9 | Codex has no channel-tag convention | RESOLVED-IN-DECISION | `docs/planning/decisions/C6-trust-rendering.md` §10 (issue #19) — layered reply correlation: (a) explicit `in_reply_to` argument, model instructed to echo `oac_message_id` verbatim; (b) adapter-independent binding to thread id and in-flight turn id via `turn/started`/`item/completed`; (c) absent (a) with more than one (b) candidate, the reply is marked `correlation inferred` or `uncorrelated`, never silently attributed; a model-supplied `in_reply_to` is validated against (b), a mismatch downgrades rather than overrides; no `ADR-001.md` text needs correction, so no new numbered amendment is issued | PLANNING-PROMPT.md §3.2 turn/event model; §3.1 Claude correlation is convention-only via `meta` |
| C10 | Permission relay lets any allowlisted sender approve tools | RESOLVED-IN-DECISION | `docs/planning/decisions/C6-trust-rendering.md` §7 (issue #19) — OAC does not declare `capabilities.experimental['claude/channel/permission']` in v0.1; justified by C10's own finding, the authenticated-but-untrusted doctrine, and the requirement not to weaken `--dangerously-load-development-channels`; the floor (`>= v2.1.234`, satisfied at pinned `v2.1.274`) is confirmed a chosen default, not an availability accident; enabling it needs its own decision record; no `ADR-001.md` text needs correction, so no new numbered amendment is issued | PLANNING-PROMPT.md §3.1 `claude/channel/permission`, v2.1.234+; `REVERIFICATION-B2.md` re-confirmed the `>= v2.1.234` floor is satisfied at `v2.1.274` (`PINS.md` Claude Code Channels pin record); PLANNING-PROMPT.md §5 decision 8 and §7 already fix the v0.1 default: permission relay **off by default in v0.1** |
